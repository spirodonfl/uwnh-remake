import { game, _testScript, _testScript_i } from "./game.js";
...
export let dom = {
  ...
  renderWorld: function () {
    ...
    for (var d = 0; d < game.getViewportData().length; ++d) {
      if (game.editor_mode) {
        var editor_block = document.querySelector(
          'div.editor_block[data-x="' +
            renderer.column +
            '"][data-y="' +
            renderer.row +
            '"]'
        ...
        coord_element.innerHTML =
          "(" + renderer.column + ", " + renderer.row + ")";
      ...
      if (game.getViewportData()[d] > 0) {
        ...
        if (!game.editor_mode) {
          ...
          var y = renderer.row;
          let element_item = document.querySelector(
            'div.blue[data-x="' + x + '"][data-y="' + y + '"]'
          );
          if (element_item) {
            element_item.remove();
          }
        ...
        if (game.editor_mode && editor.camera_has_changed) {
          // COLLISION BLOCKS
          var _artificial_layer =
            game.getCurrentWorldSize().width *
            game.getCurrentWorldSize().height *
            1;
        ...
        if (data > 0 && !dom.__world_rendered) {
          ...
          let element_entity = document.querySelector(
            'div[data-entity="' + data + '"]'
          );
          if (!element_entity) {
  ...
  sizeView: function () {
    ...
    document.getElementById("view").style.margin =
      y_padding + "px " + x_padding + "px";
    document.getElementById("view").style.width = x * (SIZE * SCALE) + "px";
    document.getElementById("view").style.height = y * (SIZE * SCALE) + "px";
    document.getElementById("clickable_view").style.width =
      x * (SIZE * SCALE) + "px";
    document.getElementById("clickable_view").style.height =
      y * (SIZE * SCALE) + "px";
    dom.width = x;
    dom.height = y;
  },
          ...
          if (mode === "entity") {
            ...
            if (values_read === collision_update.length) {
              if (collision_update[0] === 1) {
                let collision_element = document.querySelector(
                  'div[data-x="' +
                    collision_update[1] +
                    '"][data-y="' +
                    collision_update[2] +
                ...
                element_item.style.left =
                  collision_update[1] * (SIZE * SCALE) + "px";
                element_item.style.top =
                  collision_update[2] * (SIZE * SCALE) + "px";
                element_item.classList.add("blue");
              }
              mode = null;
            }
            ...
            if (viewport_update.length === 7) {
              ...
              element_item = document.querySelector(
                'div.thing[data-x="' +
                  from_coords[0] +
                  '"][data-y="' +
                  from_coords[1] +
            ...
            if (viewport_update.length === 1) {
              let element_entity = document.querySelector(
                'div[data-entity="' + viewport_update[0] + '"]'
              );
              if (element_entity) {
...
window.addEventListener("load", function () {
  ...
  element_clickable_view.addEventListener("click", function (e) {
    ...
    var clicked_element = document.querySelector(
      'div.thing[data-x="' + x + '"][data-y="' + y + '"]'
    );
