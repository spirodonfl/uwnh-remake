...
// test "read_bin" {
//     // some kind of data structure that holds files && sizes
//     // generated by build maybe?
//     const f = try std.fs.cwd().openFile("game/binaries/test_data.bin", .{});
//     const file_size = (try f.stat()).size;
//     // iterate over files and increase file_size
//     const reader = f.reader();
// 
//     // ------ FIXBUFFER MODE -------
//     var test_file = @embedFile("game/binaries/test_data.bin");
//     // for (test_file) |value| {
//     //     std.debug.print("B: {d}\n", .{value});
//     // }
//     var ii: usize = 0;
//     while (ii < test_file.len) {
//         // LITTLE ENDIAN MODE
//         var tf: u16 = (@as(u16, @intCast(test_file[ii + 1])) << 8) | @as(u16, @intCast(test_file[ii]));
//         // BIG ENDIAN MODE
//         // var tf: u16 = (@as(u16, @intCast(test_file[ii] << 8) | @as(u16, @intCast(test_file[ii + 1]));
//         std.debug.print("C: {d}\n", .{tf});
//         ii += 2;
//     }
//     var buffer: [test_file.len]u8 = undefined;
//     var fba = std.heap.FixedBufferAllocator.init(&buffer);
//     const fba_allocator = fba.allocator();
//     const fba_memory = try fba_allocator.alloc(u16, (test_file.len / 2));
//     var fba_cursor: usize = 0;
//     while (true) {
//         const i = reader.readInt(u16, .Little) catch |e| switch (e) {
//             error.EndOfStream => break,
//             else => return e,
//         };
//         fba_memory[fba_cursor] = i; // u16 (i = u16)
//         std.debug.print("A: {d}\n", .{fba_memory[fba_cursor]});
//         fba_cursor += 1;
//     }
// 
//     // ------ GPA MODE -------
