...
pub fn addRowToWorld(world_index: u16) !void {
    ...
    while (i < world.embedded_layers.items.len) {
        ...
        std.log.info("Layer length before: {d}", .{world.embedded_layers.items[i].raw_data.items.len});
        try world.embedded_layers.items[i].raw_data.appendNTimes(game.allocator, 0, world.getWidth());
        std.log.info("Layer length after: {d}", .{world.embedded_layers.items[i].raw_data.items.len});
        i += 1;
    }
