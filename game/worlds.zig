...
var current_world = current_worlds[0];
...
pub fn currentGetData(layer: u16, x: u16, y: u16) u16 {
    var index: u16 = world_indexes[current_world];
    var size: u16 = world_sizes[current_world];
    var width: u16 = world_dimensions[(current_world * 2)];
    index = index + (size * layer);
    index = index + ((y * width) + x);
    return all_worlds[index];
}
pub fn currentGetWidth() u16 {
    return world_dimensions[(current_world * 2)];
}
pub fn currentGetHeight() u16 {
    return world_dimensions[(current_world * 2) + 1];
}
